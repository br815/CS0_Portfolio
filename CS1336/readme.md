...
```
<!-- CODE END -->
This compilation instruction is intended to produce an executable in a `build` folder (by using the `-o` flag and providing the relative file path `../build/hw`). A `build` folder must be created first if it does not exist (using `mkdir`).
