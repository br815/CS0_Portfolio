...
```
<!-- CODE END -->
This compilation instruction is intended to produce an executable in the `build` folder (by using the `-o` flag and providing the relative file path `../build/hw`). The `build` folder must be created first if it does not exist (using `mkdir`).
